  ...
  return (
    <div className="relative">
      ...
      {photoUrl ? (
        ...
        // No photo: compact pill instead of an empty hero box.
        <div className="px-4 pt-3">
          <button
            type="button"
            onClick={handleTap}
            disabled={disabled || isUploading}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest text-muted bg-surface border border-subtle border-dashed hover:text-content hover:border-accent/50 disabled:opacity-50 active:scale-95 transition-all"
          >
            {isUploading ? (
            ...
            {disabled ? 'Enter SKU first' : 'Add photo'}
          </button>
        </div>
      )}
      ...
      {showActions && (
        <>
          <div className="fixed inset-0 z-[60] bg-main/60" onClick={() => setShowActions(false)} />
