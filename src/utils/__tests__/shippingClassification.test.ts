...
describe('autoClassifyShippingType', () => {
  ...
  it('a single 60 lb item still ships fedex — weight does not decide this', () => {
    const items = [{ sku: 'SKU-HEAVY', pickingQty: 1, sku_metadata: { is_bike: false } }];
    expect(autoClassifyShippingType(items, EMPTY_LOOKUP)).toBe('fedex');
  });

  it('a heavy part riding along with light items does not force a truck', () => {
    const items = [
      { sku: '03-1000BL', pickingQty: 1, sku_metadata: { is_bike: true } },
      { sku: '98-6860', pickingQty: 2, sku_metadata: { is_bike: false } },
      { sku: 'SKU-HEAVY', pickingQty: 1, sku_metadata: { is_bike: false } },
    ];
    expect(autoClassifyShippingType(items, EMPTY_LOOKUP)).toBe('fedex');
  });

  it('five bikes still ship regular, however light they are — it is volume', () => {
    const items = [{ sku: '03-1000BL', pickingQty: 5, sku_metadata: { is_bike: true } }];
    expect(autoClassifyShippingType(items, EMPTY_LOOKUP)).toBe('regular');
  });
...
describe('autoClassifyShippingType with a bikeSkus lookup (raw watchdog items)', () => {
  ...
  it('counts bikes via a Set lookup when items carry no metadata', () => {
    ...
    expect(autoClassifyShippingType(items, new Set(['06-4438BK', '06-4454BK', '07-3689WH']))).toBe(
      'regular'
    );
  });
