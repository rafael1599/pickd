...
- [ ] **15 SKUs con estantes de cantidad idéntica: ¿filas duplicadas de la carga del 27 mar?** — En 11 de los 15 el total del AS400 coincide EXACTO con un subconjunto de los estantes: `66-0110BK` (E11:514 + D11/D16/D17:65 c/u, AS400 = 514), `23-0148` (E13/E3/E4:134 c/u + E8:46, AS400 = 46), `66-0110SL` (AS400 263 = 132+131, sobran tres de 35), `66-0109BK` (AS400 192 = 96+96, sobran tres de 100), `20-0625`, `31-0250BL`, `66-0524`, `66-0784`, `66-1183BK`, `01-0518`, `01-0554`. Ninguna de esas filas tiene una sola línea en `inventory_logs` ni se ha movido desde el 27 mar 2026. **Nada se ha quitado**: las 46 filas llevan el mapa completo y qué estantes sobrarían en su `internal_note`. Serían **1.281 unidades**. Rafael (13 sep) se inclina por la hipótesis, pero no se borró porque hay una segunda lectura que encaja igual: que E11 se contara y entrara al AS400 y que D11/D16/D17 se surtieran después sin entrar — el AS400 tampoco es un conteo en vivo. **Se decide contando esos estantes**; si E13/E3/E4 tienen los tres 134 stems, la hipótesis muere ahí. Las 4 que no cuadran con ningún subconjunto (`66-0109SL`, `66-0260RD`, `66-0786`, `66-1183SL`) ya están resueltas por la regla: gana PickD. <!-- id: idea-188 --> — input: 2026-09-13 NY
- [ ] **397 SKUs donde PickD tiene más que el AS400 — conteo de piso** — 25.829 unidades de diferencia; 174 de ellos no se han tocado desde la carga inicial del 27 mar 2026 y 79 están repartidos en varios estantes. Por la regla del operador (13 sep: «mayor cantidad gana en donde no se puede decidir… en el piso se contará cuando toque») **gana PickD y no se escribe nada**. Salen del informe que genera `scripts/reconcile-from-as400.mjs --out`. Pendiente: una hoja de conteo priorizada por unidades en juego y agrupada por pasillo, empezando por idea-188. <!-- id: idea-189 --> — input: 2026-09-13 NY
- [ ] **Contenedor 7006N — verificar el color de las dos Ventura A1 al recibir** — `03-4983GY` (65 u, talla 58) y `03-4984GY` (10 u, talla 61), nombradas «Ventura A1 58/61 **Cerulean**» pero con sufijo `GY` (gris). El AS400 no conoce ningún `03-4983GY`/`03-4984GY`; sí tiene `03-4983BL` y `03-4984BL` = «VENTURA A1 CUES 2X10 58/61 2026 CERULEAN», que fueron stock real en ROW 27 hasta agotarse en abril. Las otras cuatro tallas del mismo contenedor (`03-4979BL`…`03-4982BL`, 48/51/54/56) sí están en el AS400. **Pero** las diez filas entraron en la misma carga masiva (10 sep 20:48:40.336, «Warehouse Team»), así que el `GY` viene del documento de origen, no de un tecleo; y en la numeración de Jamis (base = modelo+talla, sufijo = color) `03-4983GY` sería la forma válida de un color nuevo que el AS400 aún no ha dado de alta. Operador (13 sep): **esperar al contenedor** — 7006N no tiene orden de recorrido, nadie recoge de ahí. **Al abrirlo:** si son celestes → renombrar `GY`→`BL` con `rename_sku_everywhere` (las 75 u están en 7006N y `03-4983BL` no tiene fila ahí, así que la fila se renombra en su sitio; ningún pedido abierto las menciona); si son grises → pedir a compras el alta de `03-4983GY`/`03-4984GY` en el AS400. Señal a favor del error: en ese mismo contenedor `03-4704GY` se llama «RENEGADE S3 CUES 56 2026 **PRIMER GREY**» — cuando es gris, el nombre lo dice. <!-- id: idea-186 --> — input: 2026-09-13 NY
- [ ] **Contenedor 7005N/7006N — el AS400 no tiene creadas dos tallas de Renegade S4** — `03-4711BR` (56", 45 u) y `03-4712BR` (58", 30 u), Copper Tone. Aquí el sufijo **sí** concuerda: `03-4708BR`/`4709BR`/`4710BR`/`4713BR` (48/51/54/61) están todas en el AS400. Faltan los números base 4711 y 4712 bajo cualquier sufijo, o sea mercancía sin dar de alta, no un SKU equivocado — **no renombrar**. Acción: compras pide el alta. <!-- id: idea-187 --> — input: 2026-09-13 NY
- [ ] **¿El piso cuenta piezas o pares? — 18 SKUs que el AS400 no vende por unidad suelta** — `unit_meas` del AS400 dice `PR`/`PAIR` en 73 SKUs y `SET` en 10 (más KIT, BOX, BAG, ROLL, JAR, BOTT). La reconciliación del 12–13 sep copió el `On Hand` del AS400 como si fueran piezas sueltas en **18 de ellos: 3.291 unidades**. Sospechoso principal: `28-1001 SEATPIN JAMIS QR SEATLUG ADAPTERS **PAIR**`, PickD tenía 99 y quedó en 200 — casi exactamente el doble. Otros: `37-0300` (879), `37-0400CH` (764), `37-0400BK` (467), `66-3002` (290), `66-0372BK` (237), `66-0372SL` (203), `10-0222BK` (166), `66-0623` KIT (126), `66-0651BK` (90), `66-0652BK`/`66-0652SL` (64), `66-0375BK` (50), `20-0625` (31), `65-1000` (25), `10-0223BK` (10), `28-0313` SET (9), `66-0518` (1). **Urgencia baja, corrección posible**: 17 de los 18 tienen cero salidas en 60 días. Se decide contando un estante: si `37-0300` tiene 879 bolsas o 879 tapones. <!-- id: idea-193 --> — input: 2026-09-14 NY
- [ ] **Precios del AS400: 1.897 fichas sin traducir** — los 1.927 SKUs leídos traen la escalera completa (`Each` / `Level 1` / `Level 2` / `Level 3` con cortes de cantidad) en `as400_snapshot.price_breaks`, y **nunca se ha escrito ninguna** a `sku_metadata`: el PRD del enriquecimiento los declaró fuera de alcance («`is_bike`, `dimensions_*`, precios, stock → nunca»). En PickD sólo hay precios en dos familias, escritos a mano: `01-` (100 de 126) y `02-` (12). **Lo único que sé traducir con certeza son las S&D**: de las 90 que ya tienen `sd_price`, **88 coinciden al centavo** con el `Each` del AS400 — quedan **36 fichas S&D** con precio en el AS400 y `sd_price` vacío, que es trabajo seguro. Para las ~1.797 restantes la escalera es descendente por cantidad (`680.95` @49 · `640.95` @99 · `620.95` @199), o sea mayorista, y el único campo de precio que enseña la ficha de inventario es `sd_price` («lo que vale esta unidad»): meter ahí una tarifa de mayorista sería peor que dejarlo vacío. **Bloqueado por una decisión de negocio**: qué es cada nivel. <!-- id: idea-194 --> — input: 2026-09-14 NY
...
- [ ] **Tres bicis juveniles marcadas como parte** — `01-8771SL` LASER 1.6 (2 u), `01-8803GN` LASER 16 MEAN (2 u) y `01-0601` S/D STARLITE (1 u) tienen `is_bike = false` en el catálogo, así que el nuevo contenedor *Small bikes* no las ve: caen en **Parts**, que es donde acaban de todas formas, pero el número de bicis de Ship no las cuenta y la etiqueta no les pone la unidad de talla. Aparecieron al validar `isSmallBikeSku` (15 sep 2026): son las únicas tres filas que el clasificador marca pequeñas y el `is_bike` contradice. Relacionado con idea-195. <!-- id: idea-213 --> — input: 2026-09-15 NY
