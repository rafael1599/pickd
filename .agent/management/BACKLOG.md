...
- [ ] **Where to put? — rediseño completo (autocomplete + solo ROW + panel velocidad)** — `PlaceSkuTab.tsx` hoy: (1) busca por SKU exact-match (`eq('sku', x)`) sin sugerencias, (2) corre suggestions aunque el SKU no exista en inventory, (3) lista cualquier `location` (no solo ROW), (4) no muestra contexto de venta del SKU. Rediseño en 4 capas:
  1. **Autocomplete:** input pasa por `useDebounce(200)` → hook nuevo `useSkuSearch(query)` que llama RPC nueva `search_skus(p_text, p_limit=10)` (ilike sobre `sku_metadata.sku` + `inventory.item_name` agregado por SKU, ordenado por `orders_30d DESC`). Render usa el `AutocompleteInput` reusable. Hasta que el usuario seleccione un SKU concreto, no se llama `suggest_locations_for_sku`.
  2. **Inventario obligatorio:** post-selección, validar que el SKU TIENE inventory activo. Si no, mostrar mensaje "Este SKU no tiene stock — usa 'New Item' para registrarlo" en vez de listar suggestions vacías.
  3. **Solo ROW como targets:** modificar `suggest_locations_for_sku` (migración nueva) — agregar filtro `WHERE l.location ILIKE 'ROW%'` en el CTE `loc_summary`. Excluye M-slots, FDX RETURNS, shipping areas que hoy se cuelan por accidente.
  4. **Panel SKU context:** debajo del autocomplete, una mini-card mostrando `sku_velocity_tier`, `sku_orders_30d`, `sku_orders_90d`, `sku_total_qty` (todos ya retornados por la RPC), + última fecha de orden (nueva query a `picking_lists` con `MAX(updated_at) WHERE status='completed'`). Velocity tier coloreado (HOT rojo / WARM ámbar / COLD azul).
  **Prereq:** idea-117 (filtros hidden-rows ya sirven aquí).
  ~4-6h. <!-- id: idea-118 -->

- [ ] **Combined orders — suprimir warnings cruzados con órdenes del mismo grupo** — Bug operativo: cuando varias órdenes se combinan en un grupo (`order_groups`), `useWaitingConflicts.ts:37-75` sigue tratando cada `picking_list` hermana como "otra orden" → muestra "this SKU is needed in another order" para items que ya están dentro de la combinada → mensajes falsos al picker. **Fix:** (a) en línea 42, agregar `group_id` al `.select()`; (b) pasar `myGroupId` como parámetro a `useWaitingConflicts` (DoubleCheckView ~1100 ya tiene el dato); (c) en el loop (líneas 52-56), agregar `if (myGroupId && waitingOrder.group_id === myGroupId) continue;`. **Auditoría adicional:** sweep en `usePickingActions.ts`, badges en `InventoryCard.tsx`, y cualquier otra UI que diga "in another order" / "reserved by another order" — todas deben filtrar por group_id. Test manual: combinar 2 órdenes que compartan 1 SKU, verificar que no aparece el warning cross-order dentro de la combinada. ~3-4h. <!-- id: idea-119 -->
