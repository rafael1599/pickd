...
- [ ] **"Where to put?" logic al marcar SKU en Send to slow / Bring to active** — El operador quiere que cuando marca un SKU en Send to slow (o Bring to active), se le ofrezcan las mismas opciones de destino rankeadas que da "Where to put?" (la RPC `suggest_locations_for_sku`). Hoy en esos tabs el flujo de move usa `ConsolidationMoveModal` con target rows hardcoded (CONSOLIDATE_TARGETS / PROMOTE_TARGETS). Diseñar la mejor manera de mostrar las suggestions rankeadas dentro de esos tabs — opciones: (a) inyectar la lista ranked en el ConsolidationMoveModal cuando se abre desde estos tabs, reemplazando los target chips hardcoded; (b) un sub-panel "smart destinations" al seleccionar el SKU. Reusar la pieza de suggestions ya construida en PlaceSkuTab (extraer a componente compartido). Prereq: idea-121 (componente SKU compartido ayuda). Pendiente de diseño UX antes de implementar. <!-- id: idea-122 -->
