...
- [x] ~~**Bike/Part selector en "New Item"**~~ ✅ 2026-05-28 — El toggle Bike/Part de `ItemDetailView` (antes solo en `mode === 'edit'`, donde hacía `update` directo a sku_metadata por sku) ahora también aparece en `mode === 'add'`. En add se respalda con estado local `typeIsBike` (default bike) y se persiste al guardar incluyendo `is_bike` en la llamada `updateSKUMetadata` (→ `upsertMetadata`). En edit mantiene el update inmediato + ahora también setea el estado local. El botón S/D queda edit-only (necesita un SKU existente para abrir el catálogo S/D). Resuelve el caso de idea-123: al registrar un UNREG desde Double Check, el operador puede clasificarlo bike/part en el acto. **Out of scope:** la opción "Unknown" del spec original — default bike cubre el 99% y Part es un click. <!-- id: idea-068 -->
- [x] ~~**DoubleCheckView — long-press en item UNREG abre New Item prellenado**~~ ✅ 2026-05-28 — Cuando una orden trae un item que NO está en el inventario de la DB pero sí físicamente en el piso (`sku_not_found` / UNREG), mantener presionado el item (long-press 500ms, ya existía para editar) ahora abre el form de New Item (`ItemDetailView` mode `add`) prellenado con sku + item_name de la orden, para que el operador solo complete lo que falta. Antes el `else` del long-press solo mostraba `toast.error('Item not found in inventory')`. Cambios: (1) `ItemDetailView` add-mode ahora siembra el `reset()` desde `initialData` cuando se pasa (antes ignoraba initialData en add); (2) DoubleCheckView agrega `addItem` de `useInventory` al `editCallbacksRef` y, en el branch sin inventario, abre el modal `item-detail` mode `add` con prefill + `onSave` → `addItem(warehouse, formData)`. <!-- id: idea-123 -->
...
- [ ] **Consolidation — filtros adicionales recomendados** — Follow-up de idea-117 (MVP). Filtros opcionales que el operador puede activar/desactivar por tab (mismo patrón localStorage):
  ...
  Implementación: extender `HiddenRowsPicker` con sección "More filters" debajo del grid de rows, cada filtro como toggle persistido independiente. <!-- id: idea-120 -->
...
- [ ] **SKU info card — estilo unificado cross-window** — Hoy cada ventana muestra la info de un SKU con un layout distinto: el header de PlaceSkuTab (velocity tier + orders 30/90d + stock + last order), las InventoryCard del stock view, las ConsolidationCard, el tooltip de DoubleCheckView, ItemDetailView. El operador pidió que **el estilo sea consistente** en todas partes — cada ventana sigue mostrando la info más afín a su contexto, pero la presentación (chips de velocity, formato de stock, badges de location/sublocation, tipografía) debe verse igual. Plan: extraer un componente compartido `<SkuInfoHeader>` (o set de sub-componentes: `<VelocityChip>`, `<StockBadge>`, `<LocationBadge>`, `<LastOrderPill>`) en `src/components/sku/` reutilizable. Cada consumidor pasa los props que tiene; el componente decide qué renderizar. Cuidado con no romper los layouts existentes (Stock card es denso, PlaceSkuTab es amplio). Requiere inventariar los 5 sitios y definir el design system mínimo antes de tocar. ~4-6h. <!-- id: idea-121 --> — Las 4 tabs (Send to slow, Bring to active, Where to put, Clear a row) sirven candidatos basados en sets hardcoded (`CONSOLIDATE_TARGETS` ROW 20-31, `PROMOTE_TARGETS` ROW 1-10+16, etc.) sin posibilidad de excluir. Operativo reportó querer ver ROW 21-27 en Send to slow que hoy no aparecen (verificar si es por capacity, DEEP_SLOW exclusion, o full state). **Solución:** UI de chips multi-select "Rows to hide" por tab, persistido en `localStorage` con key `consolidation_hidden_rows_{tab}` (patrón consistente con `kb_pref_*` ya en uso). Se aplica como filtro post-RPC en cliente (no tocar RPCs). **Filtros recomendados adicionales** (todos opt-in, default OFF salvo bikes-only que es hidden+ON tras idea-115):
