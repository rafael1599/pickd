...
- [ ] **Consolidation — filtros adicionales recomendados** — Follow-up de idea-117 (MVP). Filtros opcionales que el operador puede activar/desactivar por tab (mismo patrón localStorage):
  ...
  **Decisión 2026-05-28:** de los 4 filtros propuestos solo **Hide full rows** valía la pena y se implementó (✅ abajo). Los otros 3 quedaron descartados: Only-ROW ya está hardcoded en el RPC (idea-118); Velocity-match quedó obsoleto tras el rework a picking_order (el ranking ya encode la velocidad); Hide-empty es contraproducente en Where to put? (las rows vacías son buenos destinos). <!-- id: idea-120 -->
- [x] ~~**Consolidation — filtro "Hide full rows" en DestinationList**~~ ✅ 2026-05-28 — Toggle que esconde rows con `free_units = 0` (destinos imposibles que solo ocupan espacio en la lista rankeada). Default ON. Persistido en localStorage `hide_full_{key}` por contexto. Vive en `DestinationList` así que aplica a Where to put? y a Send to slow / Bring to active inline. <!-- id: idea-124 -->
