-- idea-118: restrict "Where to put?" destination suggestions to ROW locations.
--
-- The prior version of suggest_locations_for_sku scored every active,
-- non-shipping location — which let M-slots, FDX RETURNS bins, and other
-- non-ROW areas leak into the ranked list. Consolidation only ever places
-- bikes into numbered ROWs, so anything else is noise.
--
-- Fix: add `AND l.location ILIKE 'ROW%'` to the loc_summary CTE. Everything
-- else identical to 20260527095915 (ambiguous-column + varchar-cast fixes).
...
BEGIN
  ...
  WITH loc_summary AS (
    ...
    WHERE l.is_active IS NOT FALSE
      ...
      -- idea-118: ROW destinations only. Excludes M-slots, FDX RETURNS,
      -- and any non-ROW area that should never receive consolidated bikes.
