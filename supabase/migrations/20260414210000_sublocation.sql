-- idea-024: Sub-locations alfabeticas por ROW
-- Adds sublocation column to inventory for fine-grained placement within ROWs.
-- Validated: only A-Z letters (1-3 chars), only for ROW% locations.

-- 1. Column
ALTER TABLE inventory ADD COLUMN sublocation text DEFAULT NULL;

-- 2. Format check: only uppercase A-Z, 1-3 chars
ALTER TABLE inventory ADD CONSTRAINT sublocation_format
  CHECK (sublocation IS NULL OR sublocation ~ '^[A-Z]{1,3}$');

-- 3. Only allowed on ROW locations
ALTER TABLE inventory ADD CONSTRAINT sublocation_only_rows
  CHECK (sublocation IS NULL OR location ILIKE 'ROW%');

-- 4. Composite index for sorting and picking path
CREATE INDEX idx_inventory_location_sublocation
  ON inventory (warehouse, location, sublocation)
  WHERE is_active = true;

-- 5. Update move RPC to accept sublocation and auto-clear on non-ROW destinations
...
BEGIN
  ...
  IF p_to_location IS NOT NULL AND p_to_location ILIKE 'ROW%' THEN
    v_resolved_sublocation := NULLIF(TRIM(UPPER(p_sublocation)), '');
  ELSE
  ...
  -- Clear sublocation on source if it still exists (qty > 0 remaining)
  -- Source sublocation stays as-is (it's already at a ROW)
