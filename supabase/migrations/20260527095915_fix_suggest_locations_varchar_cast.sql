-- Follow-up fix to 20260527085200: after resolving the ambiguous `location`,
-- the next runtime error surfaced — `locations.location` and `locations.zone`
-- are varchar(100), but RETURNS TABLE declares them as `text`. Postgres raises
-- 42804 "structure of query does not match function result type".
--
-- Fix: cast both columns to `text` inside the CTEs.
