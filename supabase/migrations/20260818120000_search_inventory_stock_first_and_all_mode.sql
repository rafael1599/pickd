-- search_inventory_with_metadata: dos correcciones a la búsqueda de inventario.
--
-- Síntoma que las motiva: buscar "ROW 33" mostraba la fila como vacía aunque
-- tenía 15 registros con 379 unidades. ROW 33 acumulaba 123 filas en qty=0
-- (fantasmas del ghost trail) cuyos SKUs ordenan antes que los vivos, así que
-- las 30 filas del primer page eran todas ceros y la pantalla — que solo pinta
-- como card lo que tiene stock — no dibujaba ninguna.
--
--   1. ORDER BY pone primero lo que tiene stock, dentro de cada ubicación.
--      Los fantasmas siguen apareciendo (el ghost trail los necesita) pero ya
--      no desplazan a los ítems reales fuera del primer page. Sin efecto en el
--      listado normal (p_include_inactive = FALSE ⇒ todas las filas tienen
--      stock y el orden por sku se mantiene).
--
--   2. p_show_parts admite NULL = "bikes y parts a la vez". La búsqueda de la
--      app quiere verlo todo y hasta ahora lo conseguía con 4 llamadas
--      (bikes/parts/S-D/FedEx) cuyos total_count se solapaban: para ROW 33
--      sumaba 244 filas donde hay 138, y el offset de "load more" se aplicaba
--      por bucket, saltándose filas. Con NULL es una sola llamada, un solo
--      total exacto y una sola paginación.
--      FALSE/TRUE se comportan igual que siempre: los demás callers no cambian.
--
-- Misma firma y mismo tipo de retorno ⇒ CREATE OR REPLACE, sin DROP.
